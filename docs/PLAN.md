...
- Verify Cayley / Schröder counts.

### Layer 1 — Tree metrics (combinatorics)
- **Robinson–Foulds** distance via split symmetric difference.
- Weighted RF (ℓ_1 and ℓ_2 on split-length vectors).
- **Quartet distance** (combinatorial, O(n²)).
...
- Implement the **GTP algorithm** (Owen–Provan) on small instances:
  - build incompatibility poset P(Σ_1, Σ_2),
...
- **Fréchet mean**: Bačák proximal iteration
  p_{k+1} = Geodesic(p_k, T_{(k mod N)+1}, 1/(k+1))
  — exploits unique BHV geodesics.
- **Sample Fréchet variance**; bootstrap confidence.
...
- Implement **tropical distance** d_∞ on ℓ_∞ quotient space.
...
| M4 | Fréchet mean + variance on real bootstrap data | 3 | 1 session |
...
- BHV geodesic results agree with published examples (Owen's paper
  Figures 4–5).
- Fréchet mean converges to the true tree on simulated bootstrap trees.
