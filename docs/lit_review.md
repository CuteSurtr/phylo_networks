...
- **Robinson–Foulds (RF)** distance: |Σ(T_1) △ Σ(T_2)|, the symmetric
...
one per binary topology. A point in orthant O(τ) with coords (l_1,…,l_{n−2})
is the tree with topology τ and interior edge lengths l_i ≥ 0.
...
- Consequence: unique geodesic between any two points; well-defined
  Fréchet means, centroids, convex hulls.
- Cone path: T → 0 → T′ is a geodesic iff the angle at origin ≥ π.
...
1. **Vogtmann decomposition:** if T_1, T_2 share split e,
   d(T_1,T_2)² = d(T_1^X,T_2^X)² + d(T_1^Y,T_2^Y)² + (|e|_{T_1} − |e|_{T_2})².
...
2. For the essential problem: enumerate candidate orthant sequences via
   the **path poset** K(Σ_1, Σ_2).

### 3.2 Combinatorial structure
- **Incompatibility poset** P(Σ_1, Σ_2): equivalence classes of Σ_2
  under identical crossing set X_{Σ_1}(·), ordered by inclusion.
- **Path poset** K(Σ_1, Σ_2): closed sets of Σ_2 under the closure
...
- **Fréchet mean** μ = argmin_p Σ d(p, T_i)² exists uniquely on any
  CAT(0) space (Sturm 2003).
- Iterative algorithms: Bačák (2014) proximal point; Sturm's law of
  large numbers for "random walks" on CAT(0) spaces.
- **Miller–Owen–Provan (2015):** polyhedral computational geometry gives
  exact Fréchet mean via locally-Euclidean approximations.
- **Brown–Owen (2020):** Fréchet variance via geodesic summation;
...
d_∞(u,v) = max_ij((u_i − v_i) − (u_j − v_j)) on the quotient
R^{(n choose 2)} / R·(1,…,1). Equivalent to the ℓ_∞ metric up to
translation.
...
| Brown_Owen_mean_variance_trees | Fréchet mean/variance |
